...
static void init(struct LinkedList* list) {
    ...
    /* list front and back sentinel not null */
	assert(list != NULL);
...
static void addLinkBefore(struct LinkedList* list, struct Link* link, TYPE value) {
    ...
    int found = 0;
    while (cur != NULL) {
        if (EQ(link, cur)) { //  link == cur) {
            found = 1;
            break;
        }
        cur = cur->next;
    }
    if (EQ(found, 0)) {// found == 0) {
        return;
    }
    ...
    if (EQ(link, list->frontSentinel->next)) { //link == list->frontSentinel->next) {
...
static void removeLink(struct LinkedList* list, struct Link* link) {
	...
	assert(link != NULL);
    /*  check for bad case 
     * deleting at the position which is out out the list 
     */
    struct Link * cur = list->frontSentinel->next;
    int found = 0;
...
int linkedListIsEmpty(struct LinkedList* deque) {
    /* FIXME: You will write this function */
	assert(deque != NULL);
    if (EQ(deque->size, 0)) {// deque->size == 0) {
...
void linkedListPrint(struct LinkedList* deque) {
    /* FIXME: You will write this function */
	assert(deque != NULL);
    if (EQ(linkedListIsEmpty(deque), 1)) {
        printf("Empty\n");
    } else {
        /* link to the front node*/
        struct Link * cur = deque->frontSentinel->next;
        /* iterate to the back node */
        while (cur != deque->backSentinel) {
...
int linkedListContains(struct LinkedList* bag, TYPE value) {
    ...
    struct Link * cur = bag->frontSentinel->next;
    /* move to the end of bag*/
    while (cur != bag->backSentinel) {
        if (EQ(cur->value, value)) { //cur->value == value) {
